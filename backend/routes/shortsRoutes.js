...
// A. ADMIN: Manage Categories (still placeholder)
...
// B. ADMIN: Upload new Pi-Shot (FIXED: order + validation)
router.post("/upload", authMiddleware, async (req, res) => {
    try {
        ...
        const exists = await Short.findOne({ ytUrl, category });
        if (exists) {
            return res.status(400).json({ message: "Video already exists in this category" });
        }

        // ✅ AUTO ORDER SYSTEM
        const last = await Short.findOne({ category }).sort({ order: -1 });
